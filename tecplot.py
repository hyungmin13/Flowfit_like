...
def Tecplotfile_gen(path, name, all_params, domain_range, output_shape, order, timestep, is_ground, is_mean, model_fn):
    ...
    uvwp = zip(*[Derivatives(dynamic_params[i,:,:,:,:], all_params, indexes, dx, dy, dz, xu, xv, xw, model_fn)
                            for i in range(time_length)])
    for i in range(timestep[0], timestep[-1]):
        if is_ground:
            ground_data = np.load(path + 'ground/ts_' + str(i).zfill(2) + '.npy')
        if is_mean:
        ...
        filename = path + 'Tecplotfile/' + name + '/ts_' + str(i) + '.dat'
        ...
        vars = [('u_pred[m/s]',np.float32(uvwp[:,0].reshape(-1))), ('v_pred[m/s]',np.float32(uvwp[:,1].reshape(-1))), 
                ('w_pred[m/s]',np.float32(uvwp[:,2].reshape(-1))),]
        if is_ground:
        ...
        np.save(path + 'npyresult/' + name + f'/ts_{i:02d}' + '.npy', np.concatenate([eval_grid_e, uvwp], axis=1))
#%%
if __name__ == "__main__":
    ...
    model = FlowFitmodel(all_params["network"]["layers"], model_fn)
